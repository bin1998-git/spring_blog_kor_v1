...
public interface ReplyRepository extends JpaRepository<Reply, Integer> {
    ...
    // 3. 전체 조회 : findAll()
    // 4. 삭제 : deleteById(Integer id) - reply의 pk
    // 5. 데이터 개수 :  count()
...
""")
    ...
    // !! 즉 board_id 로 댓글을 삭제 해주는 기능이 없다

    /**
     * 이전 수정, 삭제 기능에서는 수정은 더티 체킹으로 처리를 하였고
     * 삭제는 기본적으로 제공하는 em.remove() 메서드를 사용해서 처리 했었다
     * 지금은 직접 JPQL 쿼리를 선언해서 delete 처리하는 구문이라
     * 다른 상황이다
     * @Query(...) <- JPA가 기본적으로 SELECT 쿼리만 인식을 하기 때문에
     * INSERT, UPDATE, DELETE는 JPA에게 SELECT 쿼리가 아니라고 알려줘야
     * 제대로 동작을 함
